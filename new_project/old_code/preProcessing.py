...
class Image(object):
    ...
    def ReaddgrFile2Img(self):
        with tf.variable_scope('readdgr'):
            #self.DGR_HEADER = {}
            ...
            code_type = self.f.read(20)
            poszero = code_type.find(b'\x00')
            if(poszero != -1):
                code_type = code_type[:poszero]
            self.DGR_HEADER['CodeType'] = code_type
    ...
    def get_DGR_HEADER(self):
        return self.DGR_HEADER
...
#             self.priImg(line_data)
#         return line_labels_list,line_data_list



# if __name__ == '__main__':
#     temp_image = Image(r'/home/zhang/桌面/HIT-MW database/02 HIT-MW GT (binary)/binary_format/b04010201.dgr')