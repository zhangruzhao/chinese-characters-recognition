...
def _read_tfrecord(tfrecord_path, num_epochs=None):
    ...
    images = tf.image.decode_image(features['images'])
    images.set_shape([32, None, 3])
    # record_images = tf.decode_raw(features['images'],tf.uint8)
    # images = tf.reshape(record_images,[30,400,3])
    images = tf.cast(images, tf.float32)
    labels = tf.cast(features['labels'], tf.int32)

    #sequence_length = tf.cast(tf.shape(images)[-2] / 4, tf.int32)
    sequence_length = tf.cast(features['seq_len'],tf.int32)
    image_name = features['image_name']
    

    return images, labels, sequence_length, image_name
...
def _eval_crnn_ctc():
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for _ in range(step_nums):
            ...
            preds = _sparse_matrix_to_list(preds[0],char_dict)
            lbls = _sparse_matrix_to_list(lbls,char_dict)
            ...
            for index in range(FLAGS.batch_size):
                print('Predict {!s:s} image with gt label: {!s:s} with seq_len:{:d} <--> predict label: {!s:s}'.format(names[index], lbls[index], seq_lens[index], preds[index]))
